 ...
 *      3.如果要使用Sentinel自带的返回信息进行限流降级,则资源名为rest地址,如果资源名为@SentinelResource中value
 *          则,违法限流规则页面会报错
 *      4.如果访问了某个rest地址,sentinel控制面板中 簇点链路 里没有相关的rest url资源地址,可以给指定的方法上添加
 *          @SentinelResource(value = "xxx"),这样就可以在簇带你链路中看到相关的资源了
 */
@RestController
@Slf4j
public class FlowLimitController {
     ...
     *      使用方式是在方法上不作任何处理,在配置界面中填写资源名称为 访问url的值
     *      特别注意:如果@SentinelResource()没有blockHandler配置,不要使用配置value值进行限流,否则页面会报
     *                  com.alibaba.csp.sentinel.slots.block.flow.FlowException: null这个异常
     *              所以,配置资源名使用@SentinelResource()中value值时必须配置blockHandler
     *  测试流控规则->QPS(每秒访问量)
